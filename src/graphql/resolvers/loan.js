...
module.exports = {
  Query: {
    async getLoanRequests(_, { status }, context) {
  ...
  Mutation: {
    async createLoanRequest(_, { input }, context) {
      ...
      // Validate required fields for collateral if secured loan
      if (securityType === 'SECURED') {
        if (!collateralType || !collateralValue) {
          throw new Error('Collateral type and value are required for secured loans');
        }
      }
      ...
      const populatedLoanRequest = await LoanRequest.findById(loanRequest._id)
        .populate('borrower');
      
      // Publish new loan request event
      pubsub.publish(NEW_LOAN_REQUEST, { newLoanRequest: populatedLoanRequest });
